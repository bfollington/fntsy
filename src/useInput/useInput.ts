...
function tryGet<U, V extends { [k: string]: U }>(
  data: V,
  key: keyof V,
  def: U
) {
...
export function useInput<M extends Mapping>(
  mapping: M,
  callbacks?: InputCallbackMap<M>
) {
